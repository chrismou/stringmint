**Note this is pre-release and as such is subject to change before v1. However I've tagged 0.0.1 and plan to keep public method signatures
as they currently are so you should be OK to use.**

**It's been made public now as I needed it in another project so am currently dogfooding it myself. Most documentation was auto generated by 
Claude in a rush and not yet reviewed so keep that in mind. Any issues, please [log an issue](https://github.com/chrismou/stringmint/issues)**
...
generator for cases where a counter table is not wanted/needed (ie, session based uniqueness, paired with Redis).
...
Credit to Gemini AI for doing the maths for me.
...
$generator = PermutationGenerator::create(
    $pdo,
    $_ENV['STRINGMINT_SECRET'], // any secret key, store this as changing it will break the sequence
    length: 6,
);
...
Someone mentioned to me about implementing a profanity filter - Ive not done this in the intial release but might in future
using the `CallableExistenceChecker`. If I do I'll probably hunt around for a maintained third party profanity list.
